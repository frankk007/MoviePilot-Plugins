...
class P115StrgmSub(_PluginBase):
            ...
            if self._hdhive_query_mode == "playwright":
                ...
                if not self._hdhive_cookie and not (self._hdhive_username and self._hdhive_password):
                    ...
                    if effective_cookie:
                        ...
                        logger.warning("⚠️ HDHive API 模式缺少有效 Cookie，请配置 Cookie 或启用自动刷新")
                        self._hdhive_client = None
    ...
    def _search_nullbr(
        ...
        if not self._nullbr_client:
            logger.warning(f"⚠️ Nullbr 客户端未初始化，跳过 Nullbr 查询")
            return []
    ...
    def _search_pansou_tv(
        ...
        if not self._pansou_client:
            logger.warning(f"⚠️ PanSou 客户端未初始化，跳过 PanSou 查询")
            return []
    ...
    def _search_resources(
        ...
        注意：此方法主要供电影订阅使用。电视剧订阅使用 _search_single_source 进行逐源搜索。
        ...
        for source in sources:
            results = self._search_single_source(source, mediainfo, media_type, season)
            if results:
                return results
            else:
                # 打印回退日志
        ...
        for subscribe in tv_subscribes:
            ...
            try:
                ...
                for source_index, source in enumerate(enabled_sources):
                    ...
                    # 搜索当前源
                    p115_results = self._search_single_source(
                        source=source,
                        mediainfo=mediainfo,
